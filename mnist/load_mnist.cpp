...
vector<tpair> load_mnist_training(std::string const& base) {
	vector<tpair> ret;

	FILE *train_imgs = fopen((base + "/train-images-idx3-ubyte.txt").c_str(), "rb");
	if (!train_imgs) {
		perror("Could not open training images");
		throw "whatever";
	}
	
	FILE *train_lbls = fopen((base + "/train-labels-idx1-ubyte.txt").c_str(), "rb");
	if (!train_lbls) {
